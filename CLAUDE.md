...
**保養專欄文章草稿資料夾**：`文章/` 放的是 `articles.json` 每篇文章的原始草稿
（.md/.docx，含 meta description、大綱、正文、標籤），是給人看的來源稿，不是網站
直接讀取的資料（網站只讀 `articles.json`）。**從 2026-08-03 起改成日期資料夾分類**
（`文章/YYYYMMDD/`，同一天新增的文章放同一個資料夾），新增文章時：
1. 先在 `文章/` 底下建立當天日期的資料夾（沒有才建，同一天陸續加稿就沿用同一個）。
2. 把該篇的草稿檔放進這個資料夾。
3. 寫進 `articles.json` 時 `date` 欄位就用這個資料夾名稱對應的日期。
2026-08-03 之前的舊文章已經照各自的 `articles.json` `date` 補建對應資料夾搬移過一輪，
之後新增文章都直接用日期資料夾，不要再放回 `文章/` 根目錄下。
